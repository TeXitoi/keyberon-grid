...
use keyberon::action::{d, k, l, m, HoldTapAction, HoldTapConfig};
...
const C_ENTER: Action = HoldTap(&HoldTapAction {
    timeout: 200,
...
const CENTER: Action = m(&[LCtrl, Enter].as_slice());

#[rustfmt::skip]
pub static LAYERS: keyberon::layout::Layers<15, 5, 2> = [
    [
...
mod app {
    ...
    struct Shared {
        ...
        usb_class: UsbClass,
        #[lock_free]
        layout: Layout<15, 5, 2>,
    }

    #[local]
    struct Local {
        matrix: Matrix<EPin<Input<PullUp>>, EPin<Output<PushPull>>, 15, 5>,
        debouncer: Debouncer<[[bool; 15]; 5]>,
        ...
        *c.local.bus = Some(UsbBus::new(usb));
        ...
        (
            ...
            Local {
                ...
                debouncer: Debouncer::new([[false; 15]; 5], [[false; 15]; 5], 5),
                timer,
            },
            init::Monotonics(),
        )
